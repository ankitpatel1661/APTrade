...
class AndroidMarketActivityCoordinator(
    ...
    // Pie contributions (M7.3 Task 3, mirroring desktop's M7.2 Task 12). [executeDueContributions]
    // is the non-throwing, per-pie-degrading catch-up engine (`ExecuteDueContributions.execute`,
    // wrapped as a plain suspend closure — same "typed use case behind a closure" shape
    // [fetchTodaysOwnEarnings] already establishes). [notifyPieContribution] mirrors
    // [notifyEarnings]'s "raw typed event -> Unit closure, no L10n access here" shape exactly:
    // AppGraph's `marketActivityCoordinator` factory resolves the executed/skipped title+body
    // via `tr`/`trf` and calls into `AndroidAlertNotifier.notifyPieContribution`.
    ...
    // Dividend crediting (M8.3 Task 4, mirroring desktop's M8.2 Task 10). [processDueDividends]
    // is the non-throwing, per-symbol-degrading crediting engine (`ProcessDueDividends.execute`),
    // wrapped as a plain suspend closure — same "typed use case behind a closure" shape
    // [executeDueContributions] already establishes. [notifyDividendOutcome]/
    // [notifyDividendBackfillSummary] mirror [notifyPieContribution]'s "raw typed value -> Unit
    // closure, no L10n access here" shape: AppGraph's `marketActivityCoordinator` factory
    // resolves the cash/DRIP/backfill-summary title+body via `tr`/`trf` and calls into
    // `AndroidAlertNotifier.notifyDividend`. Two closures (not one) because the backfill-collapse
    // summary is NOT a single `DividendOutcome` — it's an aggregate (count + summed cash) this
    // class computes itself; see [notifyDividendsDue]. Direct port of desktop's
    // `DesktopMarketActivityCoordinator` dividend closures.
    ...
    // instant. Real callers (AppGraph) pass the same shared instance used everywhere else.
     ...
     * already running when the market opens. Direct port of
     * `DesktopMarketActivityCoordinator.start()`.
     */
    fun start() {
        if (job != null) return
        job = scope.launch {
     ...
     *  closure passed in here could still throw. Direct port of desktop's
     *  `runContributionCatchUpIfEnabled`. */
     ...
     * Direct port of desktop's `runDividendCatchUp`.
     *
     * CARRY-NOTE 4 (Kotlin-only divergence from the Swift AS-BUILT `run()`, recorded per M8.2
     * Task 10's brief, transcribed here for the Android port): after crediting, marks the
     * scheduler's `lastDividendDay` to today's trading day so the SAME trading day's first
     * `tick()` — which may still observe the market transitioning to OPEN later in the day —
     * does not redundantly fire a second full Yahoo dividend sweep via `DividendCheckDue`.
     * Swift's `run()` has no equivalent guard (its launch catch-up and the day's first tick
     * both call `processDueDividends`, relying on the engine's own per-event ledger dedup to
     * make the second call a cheap no-op); Kotlin additionally advances the day marker here to
     * skip the redundant network round-trip entirely. Trade-off: an event Yahoo publishes
     * between launch and market open credits same-day on Swift (its unguarded second call
     * still re-sweeps and picks it up on the open tick), but only on the next trading day or
     * next launch here (≤1 trading day of added latency; the engine's own dedup still keeps
     * the eventual credit correct) — so this marker should not be "fixed" away without
     * understanding that Swift difference.
     */
        ...
        for (event in events) {
            when (event) {
                ...
                ScheduledNotification.ContributionCheckDue -> {
                    ...
                    // never drift between them (Global Constraints correction 6). Direct port
     ...
     * never to throw except [CancellationException]. Direct port of desktop's
     * `notifyDividendsDue`.
     *
     * **Backfill collapsing.** Mirrors desktop's `notifyDividendsDue()` exactly: non-backfill
