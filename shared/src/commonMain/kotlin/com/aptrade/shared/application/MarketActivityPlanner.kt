...
/** A scheduled notification the app should deliver this tick. [DigestDue],
 *  [EarningsCheckDue], and [ContributionCheckDue] carry no content — the caller builds the
 *  body from live data.
 ...
 *  poll, and survive relaunches (no duplicate digest/earnings-check/contribution-check
 *  after restarting mid-day). Mirrors macOS's `SchedulerState`. */
 ...
 *
 * Transcribed from macOS's `MarketActivityPlanner.plan(now:state:settings:)`. The four
 * settings flags ([marketOpenCloseEnabled], [newsDigestEnabled], [earningsReportsEnabled],
 * [pieContributionsEnabled]) replace the Swift `AppSettings` struct parameter one-for-one —
 * this module has no `AppSettings` port yet, so the booleans it actually reads are passed
 * directly.
 */
class MarketActivityPlanner(private val calendar: MarketCalendar = MarketCalendar()) {
    ...
    fun plan(
        ...
        // Same default-false-for-old-test-compat pattern as [earningsReportsEnabled] above
        // (unlike AppSettings.pieContributions' true default) — real callers always pass it
        // explicitly.
        pieContributionsEnabled: Boolean = false,
        ...
        var lastContributionDay = state.lastContributionDay
        ...
        if (status == MarketStatus.OPEN) {
            val day = calendar.tradingDay(nowEpochSeconds)
            if (lastContributionDay != day && pieContributionsEnabled) {
                events += ScheduledNotification.ContributionCheckDue
