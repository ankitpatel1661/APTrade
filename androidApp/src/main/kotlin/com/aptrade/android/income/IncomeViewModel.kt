 ...
 *  fractions (0.031 = 3.1%) -- the view (Task 2) owns percent formatting, same "feeds
 *  further formatting" contract every other Android VM state in this codebase follows. */
 ...
 *  payouts, per-holding breakdown, and payment history. Android twin of desktop
 *  `IncomeViewModel` (desktopApp/.../income/IncomeViewModel.kt), transcribed near-verbatim —
 *  the desktop VM is pure-Kotlin StateFlow over shared use cases with no desktop-only
 *  imports. The one adaptation is construction/scope: this VM extends androidx [ViewModel]
 *  and uses [viewModelScope] internally (Dispatchers.Main.immediate), mirroring the house
 *  Android VM convention (see [com.aptrade.android.plans.PlansViewModel]) instead of taking
 *  a constructor-injected `CoroutineScope`. Including the `buildUpcoming` stale-projection
 *  guard (`projected.exDate > asOf`).
 *
