...
class DividendForecastTest {
    ...
    /**
     * Twelve quarterly $0.25 payments over three years, repeating [flatQuarterly]'s exact
     * phase (offsets 300/209/118/27 days, then the same pattern 364 days further back, twice
     * more) — deliberately NOT an even multiple-of-91-days-back-from-`now` series.
     *
     * A naive `(11 - i) * 91` spacing puts the newest payment EXACTLY at `now`, and since four
     * 91-day gaps span only 364 days (< 365), [DividendMath.trailingAnnualPerShare]'s rolling
     * 365-day window then captures FIVE payments (not four) at that instant — a boundary
     * artifact, not real growth. Anchored one quarter earlier (27 days before `now`, exactly
     * matching [flatQuarterly]), only four payments ever fall inside any rolling 365-day
     * window, so this fixture is genuinely flat: $1.00/share/yr, zero growth, in every year.
     */
