...
class AndroidMarketActivityCoordinatorTest {
    ...
    fun tickFiresMarketOpenedTransitionAndPersistsState() = runTest {
        ...
        // 2, not 1: start()'s ungated dividend launch catch-up (M8.3 Task 4, carry-note 4)
    ...
    fun digestFetchFailureFallsBackToUpdatingMessageWithoutBreakingTheTick() = runTest {
        ...
        // launch catch-up's own save (M8.3 Task 4, carry-note 4) before the tick loop begins.
        assertEquals(2, stateStore.saveCallCount)
    }
    ...
    fun earningsFetchFailureDropsOnlyEarningsAndTheLoopSurvives() = runTest {
        ...
        // catch-up's own save (M8.3 Task 4, carry-note 4) before the tick loop begins.
        assertEquals(2, stateStore.saveCallCount)
    ...
    // MARK: - Pie contributions (M7.3 Task 3, mirroring desktop's M7.2 Task 12)
    ...
    fun contributionCheckDueOnTheFirstOpenTickExecutesCatchUpAndNotifiesPerOutcome() = runTest {
        ...
        // (M8.3 Task 4, carry-note 4) before the tick loop begins.
        assertEquals(2, stateStore.saveCallCount)
    }
    ...
    fun launchCatchUpMarksLastDividendDay_soSameDayTickDoesNotReinvokeEngine() = runTest {
        ...
        // AndroidMarketActivityCoordinator.runDividendCatchUp's KDoc): the launch catch-up
