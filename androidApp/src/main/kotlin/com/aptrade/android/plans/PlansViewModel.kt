 ...
 *  is responsible for money/percent text rendering. `nextContributionLabel` is the one
 *  field this VM pre-localizes, since it's a full sentence (`tr`/`trf`), not a
 ...
 *  screen, one-off contributions, and manual rebalancing. Android twin of desktop
 *  `PlansViewModel` (desktopApp/.../plans/PlansViewModel.kt), transcribed near-verbatim —
 *  the desktop VM is pure-Kotlin StateFlow over shared use cases with no desktop-only
 *  imports. The one adaptation is construction/scope: this VM extends androidx
 *  [ViewModel] and uses [viewModelScope] internally (Dispatchers.Main.immediate),
 *  mirroring the house Android VM convention (see [com.aptrade.android.portfolio.PortfolioViewModel]/
 *  [com.aptrade.android.calendar.CalendarViewModel]) instead of taking a constructor-injected
 *  `CoroutineScope`.
 *
 *  Every user-visible string this VM produces (the insufficient-cash error, the
 *  next-contribution label) is routed through `tr`/`trf` (`com.aptrade.android.l10n`),
 *  mirroring desktop's `tr(_:)` calls exactly. */
class PlansViewModel(
     ...
     *  second, independently-scheduled `viewModelScope.launch`. */
     ...
     *  treats identically to "no quote for this symbol" (mirrors the desktop VM's per-symbol
     *  degrade contract, which never throws). */
