 ...
 *  from a possibly-stale caller snapshot), so a single shared [BuyAsset] instance serializes
 *  concurrent buys against the same store — the second caller sees the first's write instead
 ...
 *
 *  KNOWN LIMITATION: this mutex is private to [BuyAsset] and is a SEPARATE lock from
 *  [SellAsset]'s own private mutex, even though both act on the same [PortfolioStore]. A buy
 *  and a sell in flight concurrently are therefore NOT serialized against each other and can
